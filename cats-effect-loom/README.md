...
3. The virtual thread blocks until the `IO` completes. Virtual threads make blocking cheap, so we can write direct-style code that looks synchronous but is actually async. The JVM runtime handles scheduling millions of virtual threads efficiently.
